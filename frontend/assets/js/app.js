...
document.addEventListener("DOMContentLoaded", () => {
    ...
    btnDownloadPdf.addEventListener("click", () => {
        ...
        window.requireAuthForDownload(() => {
            ...
            title: "Sign In to Download Report",
            subtitle: "Sign in or create a free account to download your official plagiarism analysis PDF report."
        });
    });
    ...
    if (btnDownloadBibtex) {
        btnDownloadBibtex.addEventListener("click", () => {
            window.requireAuthForDownload(() => {
                ...
                title: "Sign In to Export Citations",
                subtitle: "Sign in or create a free account to export formatted academic citations."
            });
        });
    }
...
async function initUserSession() {
    ...
    const signinLink = document.getElementById("profile-signin-link");
    const switchAccountItem = document.querySelector(".switch-account-item");
    ...
    function applyUserData(user) {
        ...
        if (switchAccountItem) switchAccountItem.style.display = "flex";
    ...
    if (!token) {
        ...
        if (dropNameEl) dropNameEl.textContent = "Guest Scholar";
        if (dropEmailEl) dropEmailEl.textContent = "Sign in to export & save papers";
        if (dropRoleEl) {
            dropRoleEl.textContent = "GUEST";
            dropRoleEl.style.background = "rgba(255, 255, 255, 0.08)";
            dropRoleEl.style.color = "var(--text-secondary)";
            dropRoleEl.style.border = "1px solid rgba(255, 255, 255, 0.12)";
        }
        if (signinLink) signinLink.style.display = "flex";
        if (switchAccountItem) switchAccountItem.style.display = "none";
        if (logoutItem) logoutItem.style.display = "none";
        if (adminNav) adminNav.style.display = "none";
        if (profileAdmin) profileAdmin.style.display = "none";
        return;
    }
...
function handleLogout() {
    ...
    if (typeof showToast === "function") {
        showToast("Signed out successfully. Browsing in guest mode.", "info");
    }
}
...
window.openAuthModal = function (options = {}) {
    ...
    _pendingAuthDownloadAction = typeof options.onSuccess === "function" ? options.onSuccess : null;
    ...
    if (alertEl) {
        alertEl.className = "auth-modal-alert hidden";
        alertEl.textContent = "";
...
window.requireAuthForDownload = function (callback, options = {}) {
    ...
    window.openAuthModal({
        title: options.title || "Sign In to Download",
        subtitle: options.subtitle || "Sign in or create a free account to download your research paper, citations, and analysis reports.",
        onSuccess: callback
    });
};
    ...
    try {
        ...
        if (!res.ok) {
            ...
            if (Array.isArray(errorMsg)) {
                errorMsg = errorMsg.map(d => d.msg || JSON.stringify(d)).join(", ");
            }
        ...
        setTimeout(() => {
            ...
            if (typeof _pendingAuthDownloadAction === "function") {
                ...
                _pendingAuthDownloadAction = null;
                action();
            }
        }, 500);
